...
export function createPortfolioManager(llm: LLMClient) {
  return async (state: AnalysisState): Promise<Partial<AnalysisState>> => {
    const prompt = [
      ...
      truncateField(state.pastContext, `Past lessons:`, undefined, state.outputLanguage),
      truncateField(state.investmentPlan, `Investment Plan:`, undefined, state.outputLanguage),
      truncateField(state.traderProposal, `Trader Proposal:`, undefined, state.outputLanguage),
      truncateField(state.riskDebate.history, `Risk Debate:`, undefined, state.outputLanguage),
      '',
